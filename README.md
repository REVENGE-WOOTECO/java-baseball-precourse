...
## :heavy_check_mark: 구현할 기능 목록
 + [x] 정답숫자 생성
   + 1~9까지 중복되지 않는 3개의 수를 생성 
 + [x] 사용자의 답 입력받아서 유효성체크
   + 예외처리
     + 숫자가 아닌경우
     + 3자리가 아닌 경우
     + 숫자가 중복되는 경우
     + 0이 포함된 경우
 + [x] 사용자의 숫자에 따라 적절한 힌트 or 정답 제공
   + 같은위치 같은숫자 - 스트라이크
   + 다른위치 같은숫자 - 볼
   + 사용자의 입력숫자가 볼, 스트라이크 어느 하나에도 해당되지 않을 시 - 낫싱
 + [x] 정답이라면 새로 게임을 시작할지 완전히 종료시킬지 선택
   + 예외처리
     + 1,2 이외의 값 입력 시 예외처리

<br>

## :heavy_check_mark: 코드 리뷰 후 수정목록
+ [ ] 주석제거
  + 이름에 의도가 드러나도록
+ [ ] 클래스 분리
    + 기준: '비슷한 기능을 하는가->재사용성이 있는가'
    + [단일책임원칙(SRP)](https://steady-coding.tistory.com/370)
+ [ ] ArrayList 타입을 상위 인터페이스인 List로 받기
    + 상위 인터페이스 타입으로 할당하는것의 장점은?
    + ArrayList에만 구현되어있는 메소드를 사용하지 못할때는 어떻게 하는가?
+ [ ] isStartOver()메소드 분리하기(하나의 역할만 하도록)
